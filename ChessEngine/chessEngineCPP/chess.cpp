...
vector<pair<pair<int, int>, pair<int, int>>> move_history;
vector<pair<pair<int, int>, pair<int, int>>> castles;
vector<pair<pair<int, int>, pair<int, int>>> en_passant_moves;
...
void make_move(const pair<int, int>& start_pos, const pair<int, int>& end_pos) {
    // Check if the move is a castling move
    auto castling_move = find(castles.begin(), castles.end(), make_pair(start_pos, end_pos));
    if (castling_move != castles.end()) {
        check_castling();
    }
    ...
    board[end_pos] = piece;
    board[start_pos] = ' ';
    move_history.push_back({start_pos, end_pos});
